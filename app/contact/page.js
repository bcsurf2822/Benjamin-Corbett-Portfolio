      ...
      <div className="relative z-10 w-full max-w-2xl bg-white rounded-2xl shadow-lg p-8 md:p-12 border border-gray-100">
        <div className="text-center mb-10">
          ...
          <p className="text-xl md:text-2xl text-gray-600 leading-relaxed">
            ...
            <span className="text-2xl md:text-3xl font-bold text-primary-dark">BUILD</span> your business!
          </p>
        </div>
        ...
        <div className="flex items-center justify-center">
          <div className="relative group flex items-center gap-3 bg-gray-50 rounded-full py-3 px-6 border border-gray-200 hover:border-primary-light transition-colors duration-300">
            <FontAwesomeIcon icon={faEnvelope} className="h-5 text-primary-dark" />
            <span className="text-gray-700">crystaledgedev22@gmail.com</span>
            <button 
              onClick={() => {
                navigator.clipboard.writeText("crystaledgedev22@gmail.com");
                document.getElementById("copy-tooltip").classList.remove("opacity-0");
                setTimeout(() => {
                  document.getElementById("copy-tooltip").classList.add("opacity-0");
                }, 2000);
              }}
...
const SocialLink = ({ link, icon, label, color }) => (
  ...
  >
    <div className={`w-14 h-14 ${color} rounded-full flex items-center justify-center mb-3 text-white`}>
      <FontAwesomeIcon
        className="h-7 transition-transform duration-300 group-hover:scale-110"
        icon={icon}
      />
    </div>
    <span className="text-gray-600 font-medium group-hover:text-primary-dark">{label}</span>
  </a>
);